...
// Prevents concurrent prefetches for the same slug
const prefetchingNow = new Set<string>();
...
export async function prefetchRemoteCache(
  ...
  if (prefetchingNow.has(slug)) return;
  prefetchingNow.add(slug);
