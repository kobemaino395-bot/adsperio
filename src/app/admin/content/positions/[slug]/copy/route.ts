...
import { copyPosition } from '@/server/content/positions';
...
export async function POST(request: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  if (!verifySessionCsrf(session, String(form.get('_csrf') ?? ''))) {
    audit({ kind: 'upload.reject', username: session.username, ip, reason: `position.copy ${slug}: csrf` });
    return adminRedirect('/admin/content/positions?error=' + encodeURIComponent('CSRF check failed'));
  }

  const result = await copyPosition(slug);
  if (!result.ok) {
    return adminRedirect('/admin/content/positions?error=' + encodeURIComponent(result.reason));
  }
  audit({ kind: 'admin.access', username: session.username, ip, path: `position.copy:${slug}->${result.position.slug}` });
  return adminRedirect('/admin/content/positions?copied=' + encodeURIComponent(result.position.slug));
}
