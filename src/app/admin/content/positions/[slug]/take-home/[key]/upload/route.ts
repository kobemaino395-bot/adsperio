...
import { isValidTakeHomeKey, saveTakeHomeFile } from '@/server/content/position-files';
...
export async function POST(request: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  if (!verifySessionCsrf(session, String(form.get('_csrf') ?? ''))) {
    audit({ kind: 'upload.reject', username: session.username, ip, reason: `position.take-home.upload ${slug}/${key}: csrf` });
  ...
  const result = await saveTakeHomeFile(slug, key, buf, file.name, file.type, session.username);
  if (!result.ok) {
    audit({ kind: 'upload.reject', username: session.username, ip, reason: `position.take-home.upload ${slug}/${key}: ${result.reason}` });
    return adminRedirect(back + '?error=' + encodeURIComponent(result.reason));
  }

  const saved = await setPositionTakeHomeFilename(slug, key, result.meta.originalFilename);
  if (!saved.ok) {
    return adminRedirect(back + '?error=' + encodeURIComponent(saved.reason));
  }

  audit({ kind: 'admin.access', username: session.username, ip, path: `position.take-home.upload:${slug}/${key}` });
