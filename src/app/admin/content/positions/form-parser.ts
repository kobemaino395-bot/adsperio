...
export function parsePositionForm(form: FormData): Partial<Position> & { slug: string } {
  ...
  return {
    ...
    googleFormUrl: String(form.get('googleFormUrl') ?? ''),
    googleFormTitle: String(form.get('googleFormTitle') ?? ''),
    googleFormBlurb: String(form.get('googleFormBlurb') ?? ''),
    showGoogleForm: form.get('showGoogleForm') === 'on',
