...
- **Take-home files**: one file per position, shared across all its roles — managed directly in `/admin/content/positions/<slug>`, no separate file manager. Uploading replaces the current file; PDF/ZIP/DOC/DOCX only, 50 MB cap. The file lives at `/var/lib/adsperio/positions/<slug>/take-home/` and is served at `/careers/<slug>/take-home`.
